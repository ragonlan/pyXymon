...
class Xymon(object):
    ...
    def send(self):
        ...
        if self.type == 'status':
            report = '{0}+{1} {2}.{3} {4} {5} \n {6}'.format(
                self.type, self.lifetime, self.host, self.test, self.color, self.title, self.text)
        else:
            report = '{0} {1}.{2} {3} \n {4}'.format(
                self.type, self.host, self.test, self.title, self.text)

        if os.environ.get('XYMON'):
            p = Popen([self.cfgvar['BB'], self.cfgvar[
                      'BBDISP'], '@'], stdout=PIPE, stdin=PIPE)
            p.communicate(report)
            logging.debug('{0} {1} {2}'.format(
                self.cfgvar['BB'], self.cfgvar['BBDISP'], report))
        else:
...
class XymonGraph(Xymon):
    ...
    def send(self):
        ...
        if os.environ.get('XYMON'):
            ...
            logging.debug('{0} {1} {2}'.format(
                self.cfgvar['BB'], self.cfgvar['BBDISP'], report.encode('utf8')))
        else:
            logging.info('{0} {1} {2}'.format(
                self.cfgvar['BB'], self.cfgvar['BBDISP'], report.encode('utf8')))
